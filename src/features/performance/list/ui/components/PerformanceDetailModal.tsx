...
export function PerformanceDetailModal({
	...
	useEffect(() => {
		const findCompany = companies?.find(r => r.id === record?.company)?.name;
		if (findCompany) {
	...
	const handleSubmit = form.handleSubmit(async (values) => {
		...
		try {
			...
			window.$message?.success("عملکرد با موفقیت ویرایش شد");
			onUpdated?.();
			onClose();
		}
		finally {
	...
	return (
		<Modal
			open={open}
			title={config?.title ?? "ویرایش عملکرد"}
			onCancel={onClose}
			footer={null}
			width={920}
			destroyOnClose
